                ...
                if self.myvar.systemCalibrate:
                    ...
                    for i in range (0, self.myvar.spinRowsWells.value()):
                        for j in range (0, self.myvar.spinColsWells.value()-1):
                            ...
                            self.myvar.arduino.write(bytes('$J=G21G91X' + str(dirXMovement*w2wDistance) + self.speed_explore, 'utf-8')) # update this by @CALSTART\r to calibrate all motors but use interrumptions
                        ...
                        self.myvar.arduino.write(bytes('$J=G21G91Y-' + str(w2wDistance) + self.speed_explore, 'utf-8')) # update this by @CALSTART\r to calibrate all motors but use interrumptions
    ...
    def nextFrameSlot(self):
        if self.isCameraOpen:
            ...
            if self.readyToSave:
                pictureSaved = False
                print("saving")
                cv2.imwrite('img_' + self.imgFileName +'%05d.jpg'%self.ith_frame, frame)
#                cv2.imwrite('img_%05d.jpg'%self.ith_frame, frame)
                time.sleep(0.2)
                self.ith_frame += 1
                pictureSaved = True
...
#        vbox.addWidget(self.capture_button)
    ...
    def manualMotorControlXPositive(self):
        if self.arduino.isOpen():
            print("+X")
            self.arduino.write(bytes('$J=G21G91X' + self.spinManualStepSize.textFromValue(self.spinManualStepSize.value()/100.0) + self.speed_rel, 'utf-8')) # update this by @CALSTART\r to calibrate all motors but use interrumptions
            time.sleep(0.10)
        else:
            print("Port is not opened")
    ...
    def manualMotorControlYPositive(self):
        if self.arduino.isOpen():
            print("+Y")
            self.arduino.write(bytes('$J=G21G91Y' + self.spinManualStepSize.textFromValue(self.spinManualStepSize.value()/100.0) + self.speed_rel, 'utf-8')) # update this by @CALSTART\r to calibrate all motors but use interrumptions
            time.sleep(0.10)
        else:
            print("Port is not opened")
    ...
    def manualMotorControlZPositive(self):
        if self.arduino.isOpen():
            print("+Z")
            self.arduino.write(bytes('$J=G21G91Z' + self.spinManualStepSize.textFromValue(self.spinManualStepSize.value()/100.0) + self.speed_rel, 'utf-8')) # update this by @CALSTART\r to calibrate all motors but use interrumptions
            time.sleep(0.10)
        else:
            print("Port is not opened")
